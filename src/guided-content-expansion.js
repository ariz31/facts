...
const REFERENCE_PATH_ID = "reference-library";
...
export function expandGuidedContent(deck, spec) {
  ...
  const referencePathIndex = deck.paths.findIndex((path) => path?.id === REFERENCE_PATH_ID);
  const referenceCardIds = new Set(referencePathIndex >= 0 ? deck.paths[referencePathIndex].cardIds ?? [] : []);
  const orderedExistingCards = [...deck.cards].sort((a, b) => Number(a.sequence) - Number(b.sequence));
  const leadingCards = referencePathIndex >= 0
    ? orderedExistingCards.filter((card) => !referenceCardIds.has(card.id))
    : orderedExistingCards;
  const referenceCards = referencePathIndex >= 0
    ? orderedExistingCards.filter((card) => referenceCardIds.has(card.id))
    : [];
  const startSequence = Math.max(0, ...leadingCards.map((card) => Number(card.sequence) || 0)) + 1;
  const guidedPaths = [];
  const guidedCards = [];
  ...
  spec.paths.forEach((path, pathIndex) => {
    ...
    guidedPaths.push({ id: path.id, title: path.title, description: path.description, cardIds });
  ...
  const shiftedReferenceCards = referenceCards.map((card, index) => ({
    ...card,
    sequence: startSequence + guidedCards.length + index,
  }));
  const pathInsertIndex = referencePathIndex >= 0 ? referencePathIndex : deck.paths.length;

  return {
    ...deck,
    estimatedMinutes: Number(deck.estimatedMinutes || 0) + spec.estimatedMinutes,
    paths: [
      ...deck.paths.slice(0, pathInsertIndex),
      ...guidedPaths,
      ...deck.paths.slice(pathInsertIndex),
    ],
    cards: [...leadingCards, ...guidedCards, ...shiftedReferenceCards],
  };
}
